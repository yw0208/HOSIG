...
from toolss.utils import aa2rotmat
from toolss.utils import makepath
from toolss.utils import to_cpu
...
def get_ground_zup(grnd_size = 5):
    g_points = np.array([[-.2, -.2, 0.0],
                         [.2, .2, 0.0],
                         [.2, -0.2, 0.0],
                         [-.2, .2, 0.0]])
    g_faces = np.array([[0, 1, 2], [0, 3, 1]])

...
from toolss.train_tools import EarlyStopping
...
from toolss.utils import makepath, makelogger, to_cpu, to_np, to_tensor, create_video
from loguru import logger

from toolss.utils import aa2rotmat, rotmat2aa, rotmul, rotate

from toolss.utils import smplx_loc2glob
...
from toolss.utils import aa2rotmat, rotmat2aa, d62rotmat
from models.model_utils import full2bone, full2bone_aa, parms_6D2full
from toolss.train_tools import v2v
from tqdm import tqdm

from toolss.vis_tools import sp_animation, get_ground, get_ground_zup
from toolss.utils import LOGGER_DEFAULT_FORMAT
...
class Trainer:
    ...
    def __init__(self,cfg, inference=False):
        ...
        model_path = os.path.join(self.body_model_cfg.get('model_path', 'data/models'))
    ...
    def infer(self, x):
        ...
        dec_x = torch.cat([v.reshape(bs, -1).type(torch.float32).to(self.device) for v in dec_x.values()], dim=1)
    ...
    def inference_generate(self):
        ...
        for obj_name in ['bottle_03', 'cup_01', 'mouse_01']:
            for batch_file_id in range(0, 9):
                batch_all = np.load(f"generate/multi_samples/batch_eval/batch_{obj_name}_{batch_file_id:02d}.npy", allow_pickle=True).item()
                sbj_vtemp = torch.from_numpy(Mesh(filename="/opt/data/private/datasets/TRUMANS/process4Gnet_3obj/tools/subject_meshes/male/s1.ply").v.astype(np.float32)).unsqueeze(0)
                for batch_id in range(2):
                    batch = {
                        'transl_obj': torch.from_numpy(batch_all['transl_obj'][batch_id]).unsqueeze(0),
                        'scene_bps': torch.from_numpy(batch_all['scene_bps'][batch_id]),
                        'betas': torch.from_numpy(batch_all['betas'][batch_id]),
                        'bps_obj_glob': torch.from_numpy(batch_all['bps_obj_glob'][batch_id]),
                        'sbj_vtemp': sbj_vtemp,
                        'gender': torch.ones(1),
                        'verts_obj': torch.from_numpy(batch_all['verts_obj'][batch_id]),
                        'global_orient_obj': batch_all['global_orient_obj'][batch_id],
                    }
                    ####
                    ...
                    sbj_m.v_template = sbj_vtemp.to(sbj_m.v_template.device)
                    # continue
                    ...
                    grasp_meshes_path = os.path.join(self.cfg.results_base_dir, f'{obj_name}_grasp_{batch_file_id}')

                    if save_meshes:
                        makepath(grasp_meshes_path)

                    # grnd_mesh, cage, axis_l = get_ground()
                    grnd_mesh = get_ground_zup()
                    for i in range(num_samples):
                        ...
                        sbj_transl = optim_output['transl'].cpu().numpy()
                        sbj_fullpose_rotmat = optim_output['fullpose_rotmat'].cpu().numpy()
                        ...
                        if visualize:
                            ...
                            time.sleep(1)
                        if save_meshes:
                            # sbj_cnet.write_ply(grasp_meshes_path + f'/{batch_id:04d}_{i:04d}_sbj_coarse.ply')
                            sbj_opt.write_ply(grasp_meshes_path + f'/{batch_id:04d}_{i:04d}_sbj_refine.ply')
                            obj_i.write_ply(grasp_meshes_path + f'/{batch_id:04d}_{i:04d}_obj.ply')

                            np.save(grasp_meshes_path + f'/{batch_id:04d}_{i:04d}_sbj_transl.npy', sbj_transl)
                            np.save(grasp_meshes_path + f'/{batch_id:04d}_{i:04d}_sbj_fullpose_rotmat.npy', sbj_fullpose_rotmat)
...
def inference():
    ...
    parser = argparse.ArgumentParser(description='GNet-Inference')
    ...
    parser.add_argument('--smplx-path',
                        default='../body_models/smplx/models',
                        type=str,
    ...
    parser.add_argument('--load_model', default='sgap/saves/GNet_3obj/snapshots/E120_model.pt', type=str,)
    ...
    if cmd_args.expr_id is None:
        cfg_path = f'{cdir}/../configs/GNet_orig.yaml'
        cfg = OmegaConf.load(cfg_path)
        cfg.best_model = f'{cdir}/../models/GNet_model.pt'
    else:
        ...
        if cmd_args.load_model is not None:
            cfg.best_model = cmd_args.load_model
    ...
    cfg.results_base_dir = os.path.join(cfg.output_folder, cfg.expr_ID, f'sample_results_{os.path.basename(cfg.best_model).split("_")[0]}')

...
from sgap.losses.sdf_loss import get_NN, get_interior



from .utils import get_reduction_method
from toolss.typing import Tensor
...
class SDFLoss(nn.Module):
    ...
    def forward(self, human_verts, human_faces, scene_verts, weight: Tensor = None,):
        # build mesh
        human_meshes = Meshes(verts=human_verts, faces=human_faces)

        # get normal
        human_normals = human_meshes.verts_normals_packed().view(-1, 10475, 3)

        # get scene and human verts distance
        nn_dist, nn_idx = get_NN(scene_verts, human_verts)

        # get scene verts inside human
        interior = get_interior(human_normals, human_verts, scene_verts, nn_idx).type(torch.bool)

        # caculate inside distance
        penetr_dist = nn_dist[interior]

        # return sdf loss
        loss=torch.mean(penetr_dist)
...
class DistanceLoss(nn.Module):
    ...
    def forward(self, human_vert, scene_vert, weight: Tensor = None,):
        dist_matrix = torch.cdist(human_vert, scene_vert, p=2)

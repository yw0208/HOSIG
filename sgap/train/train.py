...
from toolss.objectmodel import ObjectModel
from toolss.train_tools import EarlyStopping, v2v
from toolss.utils import (LOGGER_DEFAULT_FORMAT, aa2rotmat, create_video,
                          d62rotmat, makelogger, makepath, rotate, rotmat2aa,
                          rotmul, smplx_loc2glob, to_cpu, to_np, to_tensor)
from toolss.vis_tools import get_ground, sp_animation
...
class Trainer:
    ...
    def __init__(self,cfg, inference=False):
        ...
        self.male_faces=torch.from_numpy(dict(np.load("smplx/models/smplx/SMPLX_MALE.npz", allow_pickle=True))['f'].astype(np.int32)).unsqueeze(0).to(self.device)
        self.female_faces=torch.from_numpy(dict(np.load("smplx/models/smplx/SMPLX_FEMALE.npz", allow_pickle=True))['f'].astype(np.int32)).unsqueeze(0).to(self.device)
        ...
        self.epochs_completed = int(cfg.load_model.split('E')[-1].split('_')[0]) + 1
        self.cfg = cfg
        ...
        if cfg.load_model is not None:
            try:
                self._get_network().load_state_dict(torch.load(cfg.load_model, map_location=self.device), strict=False)
                self.logger('Restored trained model from %s' % cfg.load_model)
            except:
                self.logger('No model to load')

        self.bps_torch = bps_torch()

        self.start_epoch = int(cfg.load_model.split('E')[-1].split('_')[0]) 
    ...
    def loss_setup(self):
        ...
        # Scene distance loss
        scene_distance_loss_cfg = loss_cfg.get('scene_distance', {})
        self.scene_distance_loss_weight = scene_distance_loss_cfg.get('weight', 0.0)
        self.scene_distance_loss = build_loss(**scene_distance_loss_cfg)
        self.logger(f'Scene distance loss, weight: {self.scene_distance_loss},'
                    f' {self.scene_distance_loss_weight}')
        
        # SDF loss
        sdf_loss_cfg = loss_cfg.get('sdf', {})
        self.sdf_loss_weight = sdf_loss_cfg.get('weight', 0.0)
        self.sdf_loss = build_loss(**sdf_loss_cfg)
        self.logger(f'SDF loss, weight: {self.sdf_loss},'
                    f' {self.sdf_loss_weight}')
    ...
    def forward(self, x):
        ...
        enc_x['verts'] = x['verts'] # human 400 sparse vertices

        dec_x['bps_obj'] = x['bps_obj_glob'].norm(dim=-1)
        enc_x['bps_obj'] = x['bps_obj_glob'].norm(dim=-1)

        dec_x['bps_scene'] = x['scene_bps'].norm(dim=-1)
        enc_x['bps_scene'] = x['scene_bps'].norm(dim=-1)
    ...
    def prepare_rnet(self, batch, pose, trans):
        ...
        if FN > 0:
            ...
            cnet_output['f_faces_full'] = self.female_faces.repeat(f_verts.shape[0],1,1).cuda()
        ...
        if MN > 0:
            ...
            cnet_output['m_faces_full'] = self.male_faces.repeat(m_verts.shape[0],1,1).cuda()
    ...
    def train(self):
        ...
        for it, batch in enumerate(self.ds_train):
            ...
            # del batch, loss_total, losses_dict, output
            # torch.cuda.empty_cache()
    ...
    def get_loss(self, batch, batch_idx, results):
        ...
        if self.sdf_loss_weight != 0:
            losses['cnet_sdf'] = 0
            if FN > 0:
                losses['cnet_sdf'] += self.sdf_loss(cnet['f_verts_full'], cnet['f_faces_full'], batch['scene_vert'])
            if MN > 0:
                losses['cnet_sdf'] += self.sdf_loss(cnet['m_verts_full'], cnet['m_faces_full'], batch['scene_vert'])
            
            losses['cnet_sdf'] *= self.sdf_loss_weight
        ...
        # scene distance loss
        if self.scene_distance_loss_weight != 0:
            losses['cnet_scene_distance'] = 0
            if FN > 0:
                losses['cnet_scene_distance'] += self.scene_distance_loss(cnet['f_verts_full'], batch['scene_vert'])
            if MN > 0:
                losses['cnet_scene_distance'] += self.scene_distance_loss(cnet['m_verts_full'], batch['scene_vert'])

            losses['cnet_scene_distance'] *= self.scene_distance_loss_weight
    ...
    def fit(self, n_epochs=None, message=None):
        ...
        for epoch_num in range(self.start_epoch, self.start_epoch + 25):
            ...
            with torch.no_grad():
                ...
                scalars_dict = {f'train_{k}': v for k, v in train_loss_dict.items()}
                scalars_dict.update({f'eval_{k}': v for k, v in eval_loss_dict.items()})
            ...
            # 记录显存使用情况
            allocated = torch.cuda.memory_allocated() / 1e9  # 当前已分配显存（GB）
            reserved = torch.cuda.memory_reserved() / 1e9    # 当前保留显存（GB）
            print(f"Epoch {epoch_num}: Allocated={allocated:.2f} GB, Reserved={reserved:.2f} GB")
    ...
    def inference_generate(self):
        ...
        for batch_id, batch in enumerate(data):
            ...
            from toolss.gnet_optim import GNetOptim
...
def train(train_it):
    ...
    parser.add_argument('--grab-path',
                        default='/opt/data/private/datasets/TRUMANS/process4Gnet_3obj',
                        type=str,
    ...
    parser.add_argument('--expr-id', default='GNet_3obj', type=str,
                        help='Training ID')
    ...
    parser.add_argument('--load_model', default=f"save/GNet_3obj/snapshots/E{train_it*28:03d}_model.pt", type=str)

    cmd_args = parser.parse_args()

    cfg.expr_ID = cfg.expr_ID if cmd_args.expr_id is None else cmd_args.expr_id

    cfg.datasets.dataset_dir = os.path.join(cmd_args.grab_path)
    ...
    cfg.load_model = cmd_args.load_model
    # cfg.load_model = None
    ########################################

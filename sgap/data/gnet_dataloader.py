...
class LoadData(data.Dataset):
    def __init__(self,
        ...
        if 'trumans' in dataset_dir.lower():
            self.frame_objs = np.asarray([os.path.basename(name) for name in self.frame_names])
        else:
    ...
    def load_ds(self, dataset_names):
        ...
        for name in dataset_names:
            self.ds.update(np.load(name, allow_pickle=True).item())
    ...
    def load_idx(self, idx, source=None):
        ...
        out['verts_obj'] = obj_out.vertices[0].detach() # obj's vertices, for calculating offset between hand or body to obj

...
from utils.text_control_example import collate_all, collate_all_one_sample
from os.path import join as pjoin


def main(index, scene_pointcloud=None, global_start_node=None):
    args = generate_args()
    fixseed(args.seed)
    ...
    if args.text_prompt != '':
        if args.text_prompt == 'predefined':
            ...
            texts, hints = collate_all_one_sample(n_frames, args.dataset, index)
    ...
    for rep_i in range(args.num_repetitions):
        ...
        # 加上场景密集点云来做guidance
        if args.scene_guidance:
            model_kwargs['y']['scene_pointcloud'] = torch.from_numpy(scene_pointcloud).cuda().unsqueeze(0) # 1 * N * 3
            model_kwargs['y']['global_start_node'] = torch.from_numpy(global_start_node).cuda().unsqueeze(0).unsqueeze(0).unsqueeze(0) # 1 * 1 * 1 * 3
        else:
            model_kwargs['y']['scene_pointcloud'] = None
            model_kwargs['y']['global_start_node'] = None
        ...
        if model.data_rep == 'hml_vec':
            n_joints = 22 if sample.shape[1] == 359 else 21
        ...
        if args.unconstrained:
            ...
            if 'hint' in model_kwargs['y']:
                ...
                mask_joint = hint[:,:,:66].view(hint.shape[0], hint.shape[1], n_joints, 3).sum(-1) != 0
                mask_rh = hint[:,:,66:].view(hint.shape[0], hint.shape[1], 96).sum(-1)!= 0


                hint = hint * raw_std + raw_mean

                hint_joint = hint[:,:,:66].view(hint.shape[0], hint.shape[1], n_joints, 3)
                hint_rh = hint[:,:,66:].view(hint.shape[0], hint.shape[1], 96)

                hint_joint = (hint_joint * mask_joint.unsqueeze(-1)).view(hint.shape[0], hint.shape[1], -1)
                hint_rh = hint_rh * mask_rh.unsqueeze(-1)
                hint = torch.cat([hint_joint, hint_rh], dim=-1)
    ...
    if len(all_hint) != 0:
        ...
        all_hint_joint = all_hint[:,:,:66]
        results = simple_eval(all_motions, all_hint_joint, n_joints)
        print(results)
    ...
    results_dict = {'motion': all_motions, 'hand_motion' : all_hand_motions, 'text': all_text, 'lengths': all_lengths, "hint": all_hint_for_vis,
             'num_samples': args.num_samples, 'num_repetitions': args.num_repetitions}
    np.save(npy_path,
            {'motion': all_motions, 'hand_motion' : all_hand_motions, 'text': all_text, 'lengths': all_lengths, "hint": all_hint_for_vis,
             'num_samples': args.num_samples, 'num_repetitions': args.num_repetitions})

...
def quat_fk(lrot, lpos, parents):
    ...
    """
    parents_24=np.concatenate([parents[:23],parents[37:38]],axis=0)
    gp, gr = [lpos[..., :1, :]], [lrot[..., :1, :]]

    for i in range(1, len(parents)):
        if i < lrot.shape[-2]: # Used for joint 52 setting 
            gr.append(quat_mul(gr[parents[i]], lrot[..., i : i + 1, :]))

    for i in range(1, len(parents_24)):
        gp.append(
            quat_mul_vec(gr[parents_24[i]], lpos[..., i : i + 1, :]) + gp[parents_24[i]]
        )
    ...
    res = [
        ...
        np.concatenate(
            [
                gpos[..., :1, :],
                quat_mul_vec(
                    quat_inv(grot[..., parents_24[1:], :]),
                    gpos[..., 1:, :] - gpos[..., parents_24[1:], :],
                ),
            ],
            axis=-2,
        ),
...
def rotate_at_frame_w_obj(X, Q, obj_x, obj_q, trans2joint_list, parents, n_past=1, floor_z=False, use_global_human=False):
    """
    Re-orients the animation data according to the last frame of past context.
...
def rotate_at_frame_w_obj_global(obj_x, obj_q, parents, n_past=1, floor_z=False, global_q=None, global_x=None, use_global=False):
    """
    Re-orients the animation data according to the last frame of past context.
...
def rotate_root_at_frame_w_obj(X, Q, obj_x, obj_q, trans2joint_list, n_past=1, floor_z=False):
    """
    Re-orients the animation data according to the last frame of past context.

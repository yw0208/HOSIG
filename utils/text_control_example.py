...
def pelvis_rh_text_control_example(n_frames=120, raw_mean=None, raw_std=None, index=0):
    text = [
        'Stand up',
        'Pick up the bottle with right hand',
        'Move the bottle with right hand',
       'Put down the bottle with right hand',
       'Walk',
       'Sit down'
    ...
    # 应当是一个196 * 162维的轨迹
    control = np.load("demo/data/temp_results/hint.npy")
    control_joint = control[:, :66].reshape(-1, 22, 3)
    control_hand = control[:, 66:]

    # 计算mask
    mask_hint_joint = control_joint.sum(axis=-1, keepdims=True) != 0 # 196, 22, 1 非0的表示作为指导轨迹
    mask_hint_hand = control_hand.sum(axis=-1, keepdims=True) != 0 # 196, 1 非0的表示作为指导轨迹

    # normlize
    control_joint = (((control_joint - raw_mean[:22*3].reshape(22, 3)) / raw_std[:22*3].reshape(22, 3))*mask_hint_joint).reshape(-1, 66)
    control_hand = ((control_hand - raw_mean[22*3:]) / raw_std[22*3:])*mask_hint_hand

    control = np.concatenate([control_joint, control_hand], axis=1)

    control = control[None,:,:]

    return text[index:index+1], control, 0
...
def collate_all(n_frames, dataset):
    ...
    return texts, hints # text: 一个包含多个text的列表，hints：维度是bs * 196 * 162的array

def collate_all_one_sample(n_frames, dataset, index):
    if dataset == 'humanml':
        spatial_norm_path = './dataset/humanml_spatial_norm'
    elif dataset == 'kit':
        spatial_norm_path = './dataset/kit_spatial_norm'
    ...
    texts0, hints0, _ = pelvis_rh_text_control_example(n_frames, raw_mean, raw_std, index)

    return texts0, hints0 # text: 一个包含多个text的列表，hints：维度是bs * 196 * 162的array

...
from data_loaders.humanml.scripts.motion_process import recover_root_rot_pos, recover_from_ric
...
class Text2MotionDatasetV3(data.Dataset):
    ...
    def random_mask(self, joints, n_joints=22, density=1):
        # 实际上就是随机挑了一帧根关节作为condition
        if n_joints == 22:
            # humanml3d
            controllable_joints = np.array([0, 21])
        else:
            # kit
            {1:'root', 2:'BP', 3:'BT', 4:'BLN', 5:'BUN', 6:'LS', 7:'LE', 8:'LW', 9:'RS', 10:'RE', 11:'RW', 12:'LH', 13:'LK', 14:'LA', 15:'LMrot', 16:'LF', 17:'RH', 18:'RK', 19:'RA', 20:'RMrot', 21:'RF'}
            choose_one = ['root', 'BUN', 'LW', 'RW', 'LF', 'RF']
    ...
    def random_mask_rh(self, rh_pose):
        n_joints=22
        # rh_pose: length, 96
        length = rh_pose.shape[0]
        choose_seq_num = np.random.choice(length - 1, 1) + 1
        # density = 100
        density = self.density
        if density in [1, 2, 5]:
        ...
        mask_seq[choose_seq, :] = True

        # normalize
        rh_pose = (rh_pose - self.raw_mean[n_joints*3:]) / self.raw_std[n_joints*3:]
        rh_pose = rh_pose * mask_seq # 意味着，除了被选中的关节点以外，其他的关节点都被mask掉了；只有choose_seq_num帧保留
        return rh_pose # length, 96
    ...
    def random_mask_train(self, joints, n_joints=22):
        ...
        # only use one joint during training
         ...
         # 从预定的controllable_joints中随机选择一个来控制
        if random.random() < 0.6:
            choose_joint = [controllable_joints[0]] # 大概率只控制根关节
        else:
            choose_joint = list(controllable_joints) # 小概率控制根关节和腕关节

        length = joints.shape[0]
        choose_seq_num = np.random.choice(20, 1) + 1 # 只控制少量关键帧
        choose_seq = np.random.choice(length, choose_seq_num, replace=False) # 在整个序列中随机选择choose_seq_num个位置来保留
        choose_seq.sort()
        mask_seq = np.zeros((length, n_joints, 3)).astype(np.bool)
        ...
        # 1/4概率为第一帧添加所有关节控制，3/4概率为前三帧添加所有关节控制
        ...
        # normalize
        joints = (joints - self.raw_mean[:n_joints*3].reshape(n_joints, 3)) / self.raw_std[:n_joints*3].reshape(n_joints, 3)
        joints = joints * mask_seq # 意味着，除了被选中的关节点以外，其他的关节点都被mask掉了；只有choose_seq_num帧保留
        return joints # length, 22, 3
    
    def random_mask_train_rh(self, rh_pose):
        n_joints=22
        # rh_pose: length, 96
        length = rh_pose.shape[0]
        choose_seq_num = 1 # 一次只控制一个
        choose_seq = np.random.choice(length, choose_seq_num, replace=False) # 在整个序列中随机选择choose_seq_num个位置来保留
        choose_seq.sort()
        mask_seq = np.zeros((length, 96)).astype(np.bool)
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train':
            # hint = self.random_mask_train_cross(joints, n_joints)
            hint = self.random_mask_train(joints, n_joints) # length, 22, 3

        hint = hint.reshape(hint.shape[0], -1)
        if m_length < self.max_motion_length:
            hint = np.concatenate([hint, np.zeros((self.max_motion_length - m_length, hint.shape[1]))], axis=0) # max_length, 22*3
        
        # 需要随机mask的hand pose
        if self.mode == 'train':
            hint_rh = self.random_mask_train_rh(motion[:, 4 + (n_joints - 1) * 9 : 4 + (n_joints - 1) * 9 + 96]) # length, 96

        if m_length < self.max_motion_length:
            hint_rh = np.concatenate([hint_rh, np.zeros((self.max_motion_length - m_length, hint_rh.shape[1]))], axis=0) # max_length, 96

        hint = np.concatenate([hint, hint_rh], axis=1) # max_length, 22*3+96
        ...
        return None, None, caption, None, motion, m_length, None, hint
...
class Trumans(data.Dataset):
    def __init__(self, mode, datapath='./dataset/trumans_opt.txt', split="train", control_joint=0, density=100, **kwargs):
        self.mode = mode
        ...
        if mode == 'text_only':
            ...
            self.w_vectorizer = WordVectorizer(pjoin(abs_base_path, 'glove'), 'our_vab')
            self.t2m_dataset = Text2MotionDatasetV3(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer, mode, control_joint, density)
            self.num_actions = 1 # dummy placeholder
